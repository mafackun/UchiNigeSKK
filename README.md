...
`CPY_TO`と`CPY_FROM`は先頭をコマンド、後続を引数としてパースされます。
  - パイプラインやシェルクオートなどは想定していません。

`JISYO_PATH`は無効なパスが含まれると即時にパニックします。


### オプション(コマンドライン引数)

- `--version` `-v` `-V`：バージョンとビルドターゲットを表示
- `--help` `-h` `-H`：起動に関するヘルプを表示
...
- `Backspace`：カーソル左の文字を削除（行頭なら前行と結合、ミニバッファへの操作優先；後述）
- `Delete`：カーソル位置の文字を削除（本文に対してのみ有効）
